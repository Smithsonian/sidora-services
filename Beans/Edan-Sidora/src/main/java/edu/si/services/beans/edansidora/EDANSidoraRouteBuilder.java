...
//                .log(LoggingLevel.ERROR, LOG_NAME, "${header.error}\nCamel Headers:\n${headers}");
        ...
        ThreadPoolExecutor customThreadPoolExecutor = new ThreadPoolExecutor(10, Integer.parseInt(CONCURRENT_CONSUMERS), 60L, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), threadFactory);
        ...
        from("seda:edanUpdate?concurrentConsumers=" + Integer.valueOf(CONCURRENT_CONSUMERS)).routeId("edanUpdate")
                ...
                .choice()
                    ...
                    .otherwise()
                        ...
                        .choice()
                            .when().simple("${header.SiteId} == null")
                                //Headers needed for IDS push
                ...
                .split(header("idsAssetList"))
                    ...
                    .choice()
                        .when().simple("${body.isPublic} == 'Yes' && ${body.isInternal} == 'No'")
                            ...
                            .log(LoggingLevel.DEBUG, "###############################[ Get Image http uri = ${header.CamelHttpUri} ]###############################")//.stop()
                            ...
                            .log(LoggingLevel.DEBUG, "###############################[ Saving File: {{si.ct.uscbi.idsPushLocation}}/${header.idsAssetName}/${header.idsAssetImagePrefix}${header.CamelOverruleFileName}.JPG ]###############################")//.stop()
                                        ...
                                        for (IdsAsset asset : idsAssetList) {
                                            ...
                                            for (int i = 0; i < assetsList.getLength(); i++) {
                                                ...
                                                if (assetValue.equals(nodeValue)) {
                                                    inXML = false;
                                                    log.debug(assetValue + " does not equal " + nodeValue);
                                                } else {
                                                    log.debug(assetValue + " does not equal " + nodeValue);
                                                }
                                            }
        ...
        from("seda:processCtDeployment?concurrentConsumers=" + Integer.valueOf(CONCURRENT_CONSUMERS)).routeId("EdanIdsProcessCtDeployment")
                ...
                .log(LoggingLevel.DEBUG, "*******************************[ processCtDeployment Calling idsAssetUpdate ]*******************************")
