 ...
 * @author jbirkhimer
 */
public class WCS_EdanSidoraTest extends EDAN_CT_BlueprintTestSupport {

    private static String LOG_NAME = "edu.si.wcsingest";
    ...
    private static final File testManifest = new File("src/test/resources/wcs-test-deployment/deployment_manifest.xml");
    ...
    public void addImageToEdanAndIds2Test() throws Exception {
        ...
        context.getRouteDefinition("WCSAddImageToEdanAndIds2").adviceWith(context, new AdviceWithRouteBuilder() {
            @Override
        ...
        exchange.getIn().setHeader("CamelFileName", "WCS-ECU-011-D0128-I000901.jpg");
        exchange.getIn().setHeader("imageid", "WCS-ECU-011-D0128-I000901");
        exchange.getIn().setHeader("ImageSequenceID", "WCS-ECU-011-D0128-1");
    ...
    public void addImageToEdanAndIds3Test() throws Exception {
        ...
        context.getRouteDefinition("WCSAddImageToEdanAndIds3").adviceWith(context, new AdviceWithRouteBuilder() {
            @Override
            public void configure() throws Exception {
                //weaveByToString(".*edanApiBean.*").replace().log(LoggingLevel.INFO, "Skipping Sending to edanApiBean Bean");
                //weaveByToString(".*idsPushBean.*").replace().log(LoggingLevel.INFO, "Skipping Sending to idsPushBean Bean");
