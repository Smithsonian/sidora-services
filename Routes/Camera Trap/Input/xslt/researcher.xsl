            ...
            <xsl:for-each select="CameraTrapDeployment">
                ...
                <xsl:for-each select="ImageSequence">
                    ...
                    <xsl:for-each select="ResearcherIdentifications/Identification">
                        <xsl:value-of select="concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat(concat('Researcher,', string($var2_cur/CameraDeploymentID)), ','), string($var1_cur/ImageSequenceId)), ','), string($var1_cur/ImageSequenceBeginTime)), ','), string($var1_cur/ImageSequenceEndTime)), ','), '&quot;'), string(SpeciesScientificName)), '&quot;'), ','), '&quot;'), string(SpeciesCommonName)), '&quot;'), ','), ','), ','), ','), string(number(string(Count)))), ','), ','), ','), '##')"/>
                    </xsl:for-each>
                </xsl:for-each>
            </xsl:for-each>
        </researcher>
