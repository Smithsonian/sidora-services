...
public class CameraTrapAutoS3SyncRouteBuilder extends RouteBuilder {
    ...
    @PropertyInject(value = "si.ct.uscbi.enableS3Routes", defaultValue = "false")
    public Boolean enableS3Routes;

    public Boolean isEnableS3Routes() {
    ...
    public void configure() {
        ...
        if (enableS3Routes) {
            ...
            from("aws-s3://" + s3ApprovedBucketName + "?delay={{si.ct.file.pollDelay}}" +
                    "&maxMessagesPerPoll={{si.ct.file.maxMessagesPerPoll}}" +
                    "&deleteAfterRead=true")
                    .routeId("CameraTrapDeploymentsPulldownFromS3")
                    //auto pulldown route is disabled by default. enable from hawtio or spring boot rest camel route controller as necessary
                    .noAutoStartup()
            ...
            from("file:" + processDoneDirPath + "?delay={{si.ct.file.pollDelay}}" +
                    ...
                    "&moveFailed={{si.ct.external.upload.error.dir}}").autoStartup(enableS3Routes)
                    ...
                    .to("aws-s3://" + s3IngestedBucketName + "?multiPartUpload=true")
            ...
            from("file:" + processErrorDirPath + "?delay={{si.ct.file.pollDelay}}" +
                    ...
                    .setHeader("CamelAwsS3Key", simple("${header.CamelFileName}"))
                    .setHeader("CamelAwsS3ContentLength", simple("${header.CamelFileLength}"))
                    .to("aws-s3://" + s3RejectedBucketName + "?multiPartUpload=true")
