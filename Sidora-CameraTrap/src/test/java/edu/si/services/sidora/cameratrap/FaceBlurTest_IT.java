...
@Disabled
@CamelSpringBootTest
@SpringBootTest(classes = {CamelAutoConfiguration.class, FaceBlurTest_IT.Config.class},
        properties = {"logging.file.path=target/logs",
                "solr.host=http://localhost:8090/solr",
...
public class FaceBlurTest_IT {
    ...
    private static final Logger log = LoggerFactory.getLogger(FaceBlurTest_IT.class);
    ...
    @PropertyInject(value = "{{solr.host}}")
    ...
    public void loadConfig() throws Exception {
        ...
        log.info("using user: {}, password: {}", FEDORA_USER, FEDORA_PASSWORD);
    ...
    public boolean hostAvailabilityCheck(String host, int port) {
                ...
                from("direct:testPythonScript").routeId("pythonTest")
                        .choice()
                            .when().simple("${header.isCtIngest} == 'true'")
                                ...
                                .setHeader("CamelFileAbsolutePath").simple("src/test/resources/wildlife_insights_test_data/unified-test-deployment")
                                .to("reader:file")
                            .endChoice()
                            .otherwise()
    ...
    public void testUpdateDeploymentFGDC() throws Exception {
        ...
        String fusekiResult = (String) doGet(FUSEKI_URI + "?output=xml&query=" + fusekiQuery, null);
        ...
        for (int k = 0; k < deploymentPids.getLength(); k++) {
            ...
            String deploymentobjectDatastreams = (String) doGet(FEDORA_URI + "/objects/" + deploymentPID + "/datastreams?format=xml", null);
            boolean hasFGDC = xpath(deploymentobjectDatastreams, "boolean(//@dsid='FGDC')", XPathConstants.BOOLEAN);

            if (hasFGDC) {
                String deploymentRelsExt = (String) doGet(FEDORA_URI + "/objects/" + deploymentPID + "/datastreams/RELS-EXT/content", null);
                String deploymentFGDC = (String) doGet(FEDORA_URI + "/objects/" + deploymentPID + "/datastreams/FGDC/content", null);
                ...
                if (deploymentImgResourcePid == null || deploymentImgResourcePid.isEmpty()) {
                    ...
                    String objDatastreamProfile = (String) doGet(FEDORA_URI + "/objects/" + deploymentImgResourcePid + "/datastreams/OBJ?format=xml", null);
                    ...
                    if (objMIME.contains("image")) {
                        String imgObjURL = FEDORA_URI + "/objects/" + deploymentImgResourcePid + "/datastreams/OBJ/content";
                        ...
                        if (tmpImgResourceFile.exists()) {
                            ...
                            String deploymentImgResourceFITS = (String) doPost(FITS_URI + "/examine?file=" + imgObjURL, tmpImgResourceFile);
                            ...
                            Source xslt = new StreamSource(new File("target/Wildlife-Insights/updateManifestDeployment.xsl"));
...
//                        log.info("FGDC Output:\n{}", xmlOutWriter.toString());
                            transformer.transform(xmlInSource, new StreamResult(new File("src/test/resources/wildlife_insights_test_data/update-script/fgdc_output/" + deploymentPID + "_FGDC_output.xml")));
                        }
    ...
    public void testGetTestImagePids() throws Exception {
        ...
        String fileName = "target/output/ct_pidList.txt";
        log.info("File path: {}", fileName);

        MockEndpoint mockResult = context.getEndpoint("mock:result", MockEndpoint.class);

        String uri = SOLR_HOST + "/sianct/select";
...
//        String solrQuery = "projectLabel:\"Recreation Effects on mid-Atlantic Wildlife\"";
        ...
        try (BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(file, true))) {
            for (int i = 0; i < count; i++) {
                ...
                String manifest = (String) doGet(FEDORA_URI + "/objects/" + ctPID + "/datastreams/MANIFEST/content", null);
                ...
                for (int j = 0; j < manifestResultList.getLength(); j++) {
                    ...
                    FileUtils.copyURLToFile(new URL(FEDORA_URI + "/objects/" + imgPID + "/datastreams/OBJ/content"), output);*/
                    }
                }
            }
