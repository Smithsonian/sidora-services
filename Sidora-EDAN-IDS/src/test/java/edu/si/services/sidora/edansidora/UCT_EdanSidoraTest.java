...
import edu.si.services.sidora.edansidora.beans.EdanIdsException;
import edu.si.services.sidora.edansidora.model.IdsAsset;
...
public class UCT_EdanSidoraTest extends EDAN_CT_BlueprintTestSupport {
    ...
    private static final Logger log = LoggerFactory.getLogger(UCT_EdanSidoraTest.class);
    ...
    public void ctIngestMessageTest() throws Exception {
        ...
        deleteDirectory(context.resolvePropertyPlaceholders("{{si.ct.uscbi.idsPushLocation}}"));
        ...
        AdviceWithRouteBuilder.adviceWith(context, "EdanIdsProcessCtDeployment", a -> {

                a.weaveById("ctProcessGetFedoraDatastream").replace()//.setBody().simple(testDatastreamsXML);
                ...
                a.weaveById("ctProcessEdanUpdate").replace().process(new Processor() {
                    @Override
        ...
        AdviceWithRouteBuilder.adviceWith(context, "idsAssetImageUpdate", a ->
                a.weaveById("idsAssetUpdateGetFedoraOBJDatastreamContent").replace()
        ...
        AdviceWithRouteBuilder.adviceWith(context, "idsAssetXMLWriter", a ->
    ...
    public void fedoraMessageFilterTest() throws Exception {
        ...
        AdviceWithRouteBuilder.adviceWith(context, "EdanIdsStartProcessingFedoraMessage", a -> {
                a.weaveById("processFedoraGetDatastreams").replace().setBody().simple(dsXML);
                a.weaveById("processFedoraGetRELS-EXT").replace().setBody().simple(rels_extXML);
                a.weaveById("logFilteredMessage").after().to("mock:filter");
                a.weaveById("startProcessingFedoraMessage").replace().to("mock:result");
        });
    ...
    public void fedoraDeleteMessageTest() throws Exception {
        ...
        AdviceWithRouteBuilder.adviceWith(context, "EdanIdsStartProcessingFedoraMessage", a ->
                a.weaveById("processFedoraEdanDelete").after().to("mock:result").stop());

        AdviceWithRouteBuilder.adviceWith(context, "edanDelete", a -> {
                a.weaveById("deleteEdanSearchRequest").replace().setBody().simple(readFileToString(new File(KARAF_HOME + "/test-json-data/edanDeletePidSearch_response.json")));
                a.weaveById("deleteEdanRecordRequest").replace().log(LoggingLevel.INFO, "Skipping Edan Delete HTTP Call!!!").to("mock:delete").stop();
        });

        Exchange exchange = new DefaultExchange(context);
        exchange.getIn().setHeader("methodName", "purgeObject");
        //exchange.getIn().setHeader("pid", context.resolvePropertyPlaceholders("{{si.ct.namespace}}") + ":test");
    ...
    public void filterSpeciesNameTest() throws Exception {
        ...
        AdviceWithRouteBuilder.adviceWith(context, "EdanIdsProcessFedoraMessage", a -> {
                a.weaveById("processFedoraFindParentObject").remove();
                a.weaveById("processFedoraGetManifestDatastream").replace().setBody().simple(testManifestXML);
                a.weaveByType(FilterDefinition.class).before().log("Checking imageid: ${header.imageid}...");
                a.weaveByType(FilterDefinition.class).after().log(LoggingLevel.INFO, "==========================[ TO MOCK RESULT ]=============================").to("mock:result").stop();
        });

        Exchange exchange = new DefaultExchange(context);
        exchange.getIn().setHeader("SiteId", TEST_DEPLOYMENT_ID);
        ...
        template.send("seda:processFedoraMessage", exchange);
    ...
    public void updateEdanEditContentTest() throws Exception {
        ...
        AdviceWithRouteBuilder.adviceWith(context, "edanUpdate", a -> {

                a.weaveById("updateEdanSearchRequest").replace().log(LoggingLevel.INFO, "Skipping Edan ImageId Search!!!")
                        .process(new Processor() {
                            @Override
                            public void process(Exchange exchange) throws Exception {
                                Message out = exchange.getIn();
                ...
                a.weaveById("edanUpdateEditContent").replace().log(LoggingLevel.INFO, "Skipping Edan Edit Content!!!");
    ...
    public void updateEdanCreateContentTest() throws Exception {
        ...
        AdviceWithRouteBuilder.adviceWith(context, "edanUpdate", a -> {
                ...
                a.weaveById("edanUpdateCreateContent").replace().log(LoggingLevel.INFO, "Skipping Edan Create Content!!!")
                        .process(new Processor() {
                            @Override
                            public void process(Exchange exchange) throws Exception {
                                Message out = exchange.getIn();
    ...
    public void createEdanJsonContentWithEdanIdTest() throws Exception {
        ...
        AdviceWithRouteBuilder.adviceWith(context, "createEdanJsonContent", a ->
                a.weaveAddLast().to("mock:result"));
    ...
    public void idsVelocityTest() throws Exception {
        String expectedBody = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>\n" +
                "<Assets>\n" +
                "  <Asset Name=\"emammal_image_testImageId_0.JPG\" IsPublic=\"Yes\" IsInternal=\"No\" MaxSize=\"3000\" InternalMaxSize=\"4000\">emammal_image_testImageId_0</Asset>\n" +
                "  <Asset Name=\"emammal_image_testImageId_1.JPG\" IsPublic=\"Yes\" IsInternal=\"No\" MaxSize=\"3000\" InternalMaxSize=\"4000\">emammal_image_testImageId_1</Asset>\n" +
                "  <Asset Name=\"emammal_image_testImageId_2.JPG\" IsPublic=\"Yes\" IsInternal=\"No\" MaxSize=\"3000\" InternalMaxSize=\"4000\">emammal_image_testImageId_2</Asset>\n" +
                "  <Asset Name=\"emammal_image_testImageId_3.JPG\" IsPublic=\"Yes\" IsInternal=\"No\" MaxSize=\"3000\" InternalMaxSize=\"4000\">emammal_image_testImageId_3</Asset>\n" +
                "</Assets>\n";
        ...
        for (int i = 0; i < 4; i++) {
            ...
            asset.setPid("test:00"+i);
            asset.setSiteId("testSidteId");
            asset.setImageid("testImageId_"+i);
    ...
    public void idsAssetUpdateTest() throws Exception {
        String idsAssetFilePrefix = context.resolvePropertyPlaceholders("{{si.edu.idsAssetFilePrefix}}");
        String idsAssetImagePrefix = context.resolvePropertyPlaceholders("{{si.edu.idsAssetImagePrefix}}");
        String testDeployment1 = "testDeploymentId";
        String testDeployment2 = "d00001";

        String expectedAssetDir_1 = context.resolvePropertyPlaceholders("{{si.ct.uscbi.idsPushLocation}}") + idsAssetFilePrefix + testDeployment1;
        String expectedAssetDir_2 = context.resolvePropertyPlaceholders("{{si.ct.uscbi.idsPushLocation}}") + idsAssetFilePrefix + testDeployment2;
       ...
       AdviceWithRouteBuilder.adviceWith(context, "idsAssetXMLWriter", a -> {
                a.weaveById("getAssetXml").replace()
                        .process(new Processor() {
                            @Override
                            public void process(Exchange exchange) throws Exception {
                                Message out = exchange.getIn();
                ...
                a.weaveById("saveFile").after().to("mock:result");
                a.weaveAddLast().to("mock:result");
        });
    ...
    public void edanIdsExceptionTest () throws Exception {
        ...
        AdviceWithRouteBuilder.adviceWith(context, "edanHttpRequest", a -> {
                ...
                a.weaveById("edanApiSendSingleRequest").replace().process(processor);
        });
    ...
    public void httpRegexTest() {
        ...
        exchange1.getIn().setHeader("test5", "http://localhost:8080/fedora/objects/test:001/datastreams?format=xml");
        exchange1.getIn().setHeader("test5", "http://localhost:8080/fedora/objects/test:001/datastreams?format=xml");
    ...
    public void edanIdsHTTPExceptionTest () throws Exception {
        Integer minEdanHTTPRedelivery = Integer.valueOf(context.resolvePropertyPlaceholders("{{min.edan.http.redeliveries}}"));
        ...
        AdviceWithRouteBuilder.adviceWith(context, "EdanIdsProcessCtDeployment", a -> {
                ...
                a.interceptSendToEndpoint("http:*").skipSendToOriginalEndpoint().process(processor);
                a.weaveById("ctProcessEdanUpdate").replace().to("mock:result").stop();
        });
    ...
    /**
     * Testing the batch create edan records from Fedora messages
     * @throws Exception
     */
    @Test
    public void testCreate_EdanBatchAggregation_FedoraMessages() throws Exception {

        double testBatchSize = Double.parseDouble(context.resolvePropertyPlaceholders("{{si.ct.edanIds.edan.bulk.size}}"));
        double extraMessages = 22;
        int expectedBatches = (int) Math.ceil(((testBatchSize+extraMessages)/testBatchSize));
        log.info("expected number of batches = {}", expectedBatches);

        PropertiesComponent pc = (PropertiesComponent) context.getPropertiesComponent();
        Properties extra = new Properties();
        extra.put("si.ct.batchEnabled", "true");
        ...
        MockEndpoint mockResult = getMockEndpoint("mock:result");
        mockResult.expectedMessageCount((int) (testBatchSize + extraMessages));

        MockEndpoint bulkRequest = getMockEndpoint("mock:bulkRequest");
        bulkRequest.expectedMessageCount(expectedBatches);
        bulkRequest.message(0).header("edanBulkRequests").isInstanceOf(JSONArray.class);

        MockEndpoint singleRequest = getMockEndpoint("mock:singleRequest");
        singleRequest.expectedMessageCount(0);


        AdviceWithRouteBuilder.adviceWith(context, "EdanIdsProcessFedoraMessage", a -> {
                a.weaveById("processFedoraFindParentObject").remove();
                a.weaveById("processFedoraGetManifestDatastream").replace().setBody().simple(testManifestXML);
                a.weaveById("processFedoraEdanUpdate").after().to("mock:result").stop();
        });
        ...
        AdviceWithRouteBuilder.adviceWith(context, "edanHttpRequest", a -> {
                ...
                a.weaveById("edanApiSendBulkRequest").replace()
                        .process(new Processor() {
                            @Override
                            public void process(Exchange exchange) throws Exception {
                                Message out = exchange.getIn();
                                int edanBulkRequestSize = out.getHeader("edanBulkRequests", JSONArray.class).length();
                                log.warn("Bulk Request Size = {}", edanBulkRequestSize);
                            }
                        })
                        .setBody().simple("some edan response")
                        .to("mock:bulkRequest");

                a.weaveById("edanApiSendSingleRequest").replace()
                        .log(LoggingLevel.ERROR, "*** BAD *** Hit edanApiSendSingleRequest")
                        .to("mock:singleRequest");
        });


        for (int i = 0; i < (testBatchSize + extraMessages); i++) {
            Exchange exchange = new DefaultExchange(context);
            exchange.getIn().setHeader("pid", "test:00" + i);
            exchange.getIn().setHeader("dsLabel", "testRaccoonAndFox");

            template.send("seda:processFedoraMessage?waitForTaskToComplete=Never", exchange);
        }
    ...
    /**
     * Testing the batch edit edan records from Fedora messages
     * @throws Exception
     */
    @Test
    public void testEdit_EdanBatchAggregation_FedoraMessages() throws Exception {
    ...
    /**
     * Testing the batch delete edan records from Fedora messages
     * @throws Exception
     */
    @Test
    public void testDelete_EdanBatchAggregation_FedoraMessage() throws Exception {
        ...
        AdviceWithRouteBuilder.adviceWith(context, "edanDelete", a -> {
                a.weaveById("deleteEdanSearchRequest").replace().toD("velocity:file:{{karaf.home}}/test-json-data/edanDeletePidSearch_response_json.vsl");
                a.weaveAddLast().to("mock:result");
        });
                ...
                a.weaveById("edanApiSendBulkRequest").replace()
                        .process(new Processor() {
                            ...
                            public void process(Exchange exchange) throws Exception {
                                Message out = exchange.getIn();
                                int edanBulkRequestSize = out.getHeader("edanBulkRequests", JSONArray.class).length();
                                log.info("Bulk Request Size = {}", edanBulkRequestSize);
                            }
                        })
                        .to("mock:bulkRequest");

                a.weaveById("edanApiSendSingleRequest").replace()
                        .log("*** BAD *** Hit edanApiSendSingleRequest")
                        .to("mock:singleRequest");
        });
        ...
        for (int i = 0; i < (testBatchSize + extraMessages); i++) {
            Exchange exchange = new DefaultExchange(context);
            String pid = "test:000" + i;
            exchange.getIn().setHeader("pid", pid);

            template.send("seda:edanDelete?waitForTaskToComplete=Never", exchange);
        }
    ...
    /**
     * Testing the batch create edan records from CT Ingest messages
     * @throws Exception
     */
    @Test
    public void ctIngestEdanCreateContentBatchTest() throws Exception {
        ...
        for (int i = 0; i < (testBatchSize + extraMessages); i++) {
            ...
            headers.put("ManifestXML", testManifestXML);
            headers.put("pid", "test:00" + i);
            headers.put("imageid", "testRaccoonAndFox");
            headers.put("edanServiceEndpoint", "/content/v1.1/admincontent/createContent.htm");

            template.sendBodyAndHeaders("seda:edanUpdate", "Test Body", headers);
        }

        assertMockEndpointsSatisfied();

        Assertions.assertEquals((int) testBatchSize, bulkRequest.getExchanges().get(0).getIn().getHeader("edanBulkRequests", JSONArray.class).length());
        Assertions.assertEquals((int) (extraMessages/testBatchSize), bulkRequest.getExchanges().get(bulkRequest.getExchanges().size() -1).getIn().getHeader("edanBulkRequests", JSONArray.class).length());
        log.info("Stop");
    }
