...
public class TabularScanner extends AbstractIterator<CSVRecord> {
	...
	public TabularScanner(final Iterator<CSVRecord> parser, final TypeDeterminingHeuristic<?> typeStrategy,
		...
		this.rowOfTypeStrategies = generate(typeStrategy::get).limit(numColumns).collect(toList());
		...
		this.rowOfRangeStrategies = generate(rangeStrategy::get).limit(numColumns).collect(toList());
		...
		this.rowOfEnumStrategies = generate(enumStrategy::get).limit(numColumns).collect(toList());
	 ...
	 * WARNING: Do not call this more than once on a {@link TabularScanner}. The internal parser of a scanner cannot be
	 * reset.
	 * 
	 * @param limit The number of rows to scan, 0 for all rows.
	 */
	public void scan(final int limit) {
		if (limit == 0) forEachRemaining(i -> {});
		else advance(this, limit);
	}
