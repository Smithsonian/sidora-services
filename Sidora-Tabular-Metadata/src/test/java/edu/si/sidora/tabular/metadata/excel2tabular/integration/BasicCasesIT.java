...
import edu.si.sidora.tabular.metadata.excel2tabular.ExcelParsingException;
import edu.si.sidora.tabular.metadata.excel2tabular.ExcelToTabular;
import edu.si.sidora.tabular.metadata.excel2tabular.Utilities;
...
public class BasicCasesIT {
    ...
    public void testOneSheetFile() throws IOException {
        final URL inputUrl = new File("src/test/resources/excel2tabular/xls/small-test.xls").toURI().toURL();
        ...
        final URL checkFile = new File("src/test/resources/excel2tabular/tabular/small-test.csv").toURI().toURL();
    ...
    public void testBlankFile() throws IOException {
        final URL inputUrl = new File("src/test/resources/excel2tabular/xls/blank.xls").toURI().toURL();
    ...
    public void testOneSheetFileToTabbed() throws IOException {
        ...
        log.debug("Result of extraction:\n{}", Resources.toString(result, UTF_8));
        final URL checkFile = new File("src/test/resources/excel2tabular/tabular/small-test.tsv").toURI().toURL();
        log.debug("File against which we're going to check:\n{}", Resources.toString(checkFile, UTF_8));
    ...
    public void testOneSheetFileToSingleQuoted() throws IOException {
        ...
        log.debug("Result of extraction:\n{}", Resources.toString(result, UTF_8));
        final URL checkFile = new File("src/test/resources/excel2tabular/tabular/small-test-single-quoted.csv").toURI().toURL();
        log.debug("File against which we're going to check:\n{}", Resources.toString(checkFile, UTF_8));
    ...
    public void testThreeSheetFile() throws IOException {
        final URL inputUrl = new File("src/test/resources/excel2tabular/xls/three-sheet.xls").toURI().toURL();
        final List<File> results = testExcel2Tabular.process(inputUrl);
        ...
        final File[] checkFiles =
                new File("src/test/resources/excel2tabular/tabular/three-sheet-csvs").listFiles(new FilenameFilter() {
