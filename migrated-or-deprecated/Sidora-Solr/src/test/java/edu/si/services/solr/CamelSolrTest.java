...
public class CamelSolrTest extends Solr_CT_BlueprintTestSupport {
    ...
    public void setUp() throws Exception {
        ...
        context.getRegistry().bind("dataSourceReIndex", db);
    }
    ...
    public void testFedoraMessages() throws Exception {
        ...
        AdviceWithRouteBuilder.adviceWith(context, "fedoraApimUpdateSolrJob", a ->
                a.weaveById("fedoraApimGetFoxml").replace().to("velocity:file:{{karaf.home}}/fedora/foxml/test_foxml.vsl"));

        AdviceWithRouteBuilder.adviceWith(context, "createBatchSolrJob", a ->
                a.weaveById("updateSianctReindexJob").replace().log(LoggingLevel.INFO, "Skipp updating db"));

        AdviceWithRouteBuilder.adviceWith(context, "createSolrDoc", a ->
                a.weaveById("createSolrDocChoice").replace().to("mock:createDocResult"));

        AdviceWithRouteBuilder.adviceWith(context, "sidoraSolrUpdate", a ->
                a.weaveById("sendToSolr").replace().to("mock:result"));

        AdviceWithRouteBuilder.adviceWith(context, "storeReceivedJobs", a ->
                a.weaveById("printStoreReceivedJobs").replace().to("mock:end"));
    ...
    public void ctIngestMessageTest() throws Exception {
        ...
        AdviceWithRouteBuilder.adviceWith(context, "cameraTrapSolrJob", a ->
                a.weaveById("ctJobGetFoxml").replace()
        ...
        AdviceWithRouteBuilder.adviceWith(context, "sidoraSolrUpdate", a ->
                a.weaveById("sendToSolr").replace().setBody().simple("{responseHeader={status=0,QTime=20}}").to("mock:result"));
    ...
    public void testFedoraSqlPaginationTest() throws Exception {
        ...
        AdviceWithRouteBuilder.adviceWith(context, "solrReindexAll", a -> {
                a.weaveById("clearSianctReindexTable").remove();
                a.weaveById("insertSianctReindexJob").remove();
                a.weaveById("insertSolrReindexJob").remove();
                a.weaveById("solrReindexAllPidCount").replace().setHeader("pidCount").simple(TOTAL_BATCH_COUNT + "");
                a.weaveById("solrReindexAllPidBatch").replace().process(new Processor() {
                    @Override
                    public void process(Exchange exchange) throws Exception {
                ...
                a.weaveById("reindexGetFoxml").replace()
                ...
                a.weaveById("reindexCreateSianctJob").remove();
                a.weaveById("reindexCreateSolrJob").remove();
                a.weaveById("printReindexJobs").after().log("${body}").to("mock:result");
        });
